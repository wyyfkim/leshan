...
class RegisterDevice extends Component {
  ...
  handleChange(e) {
    ...
    if (this.state.current === 0 && e.target.name === 'productID') {
      this.setState({
        showProductIDInfo : false
      });
    }
  ...
  getContentForStep(step) {
    const { identifier, metadataHash, firmwareHash, metadata, firmware, productID } = this.state;
    ...
    if (step === 0) {
      ...
      return (
        <div>
          <p>
            <strong>Unique device identifier</strong> is a public key or a fingerprint of RSA/ECC public key.
          </p>
          <Input
          ...
          <Button.Group size="large">
            ...
            <Button type="primary" style = {{background: '#51B897', border: '#51B897'}} onClick={() => this.generateEthWallet()}>Generate elliptic curve key pair</Button>
          ...
          {this.state.showIdentifierInfo ?
            <div>
              <br />
              <Alert style = {{background: '#c4f5e5', border: '#51B897'}} message="You will be given private key and device configuration on the last step." type="info" showIcon />
            </div> : null}
          <br /><br />
          <p>
          ...
          <Input
              placeholder="Enter product address here"
              style={{ maxWidth: '800px' }}
              value={productID}
              name="productID"
              maxLength="66"
          ...
          <br /><br />
        <Button.Group size="large">
          <Button type="primary" style = {{background: '#51B897', border: '#51B897'}} onClick={() => this.addProductID()}>Add product ID</Button>
        </Button.Group>
          {this.state.showProductIDInfo ?
              <div>
                <br />
                <Alert style = {{background: '#c4f5e5', border: '#51B897'}} message="Product ID added" type="info" showIcon />
              </div> : null}
      ...
      return (
        <div>
          ...
          <Form>
            ...
            <FormItem>
              <Button type="primary" style = {{background: '#51B897', border: '#51B897'}} onClick={() => this.calculateMetadataHash()}>Generate</Button>
            </FormItem>
          </Form>
        </div>
      );
    ...
    if (step === 2) {
      return (
        <div>
          ...
          <br />
          <br />
          <Button type="primary" style = {{background: '#51B897', border: '#51B897'}} onClick={() => this.calculateFirmwareHash()}>Generate</Button>
        </div>
      );
    }
      ...
      return (
        <div>
          <Card title={<div>Identifier  {identifier} <a><Icon type="edit" onClick={() => this.gotoStep(0)} /></a></div>} bordered={false}>
            <div>Product ID: {productID.length > 0 ? productID : 'empty'} <a><Icon type="edit" onClick={() => this.gotoStep(0)} /></a></div>
            <div>Metadata hash: {metadataHash.length > 0 ? metadataHash : 'empty'} <a><Icon type="edit" onClick={() => this.gotoStep(1)} /></a></div>
            <div>Firmware hash: {firmwareHash.length > 0 ? firmwareHash : 'empty'} <a><Icon type="edit" onClick={() => this.gotoStep(2)} /></a></div>
    ...
    if (step === 4) {
      return (
        <div style={{ textAlign: 'center' }}>
          ...
          <br />
          <Button type="primary" style = {{background: '#51B897', border: '#51B897'}} onClick={() => this.downloadConfiguration()}>Download</Button>
        </div>
      );
    }
  }
  ...
  async createDevice() {
    const { identifier, metadataHash, firmwareHash, address, productID } = this.state;
    try {
      let instance = await DeviceManager;
      ...
      if (address !== '') {
        ...
        // identifierToSave = addressToPad.toString('hex');
      }
      const test = addHexPrefix(identifierToSave)
      console.log(typeof(test))
      console.log(test)
      console.log(typeof(productID))
      console.log(productID)
      let result = await instance.createDevice(addHexPrefix(identifierToSave), addHexPrefix(metadataHash), addHexPrefix(firmwareHash), productID, { from: getDefaultAccount() });
          ...
          <div className="steps-action">
            {
              ...
              && <Button type="primary" style = {{background: '#51B897', border: '#51B897'}} onClick={() => this.next()}>Next</Button>
            }
            {
              current === steps.length - 1
              && <Button type="primary" style = {{background: '#51B897', border: '#51B897'}} onClick={() => this.createDevice()}>Register</Button>
            }
            {
              current > 0 && current !== 4
              && (
            ...
            }
            {
              current === 4
            //   && (
            //     <Button type="primary" style = {{background: '#51B897', border: '#51B897'}}onClick={() => this.reset()}>
            //       Reset
            // </Button>
            //   )
            }
