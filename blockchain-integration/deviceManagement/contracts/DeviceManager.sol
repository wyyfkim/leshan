...
contract DeviceBase {
     ...
     */
    function createDevice(bytes32 _identifier, bytes32 _metadataHash, bytes32 _firmwareHash, bytes32 _productID) public returns (uint) {
        Device memory newDevice = Device(msg.sender, _identifier, _metadataHash, _firmwareHash, _productID);
...
contract DeviceHelper is DeviceBase {
   /**
    ...
    function getFirmwarehash(uint _deviceId) public view returns (bytes32) {
