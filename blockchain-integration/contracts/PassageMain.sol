...
contract PassageMain is PassageHelper {
    ...
    function addProductAlert(
        bytes32 _productId,
        string _alertData
    ) public productIdExists(_productId) noChildren(_productId) returns (string newAlert) {
        // Get base product from storage
        Product storage product = productIdToProductStruct[_productId];
        product.alerts = _alertData;
...
//    event productAlertAdded(bytes32 productId, bytes32 deviceID, );
    ...
    returns (string name, string description, string _latitude, string _longitude, uint versionCreationDate, bytes32[] versions, string alerts) {
//        returns (string name, string description, string _latitude, string _longitude, uint versionCreationDate, bytes32[] versions, bytes32[] certificationsIds) {
      ...
      return (product.name, product.description, requestedVersion.latitude, requestedVersion.longitude, requestedVersion.creationDate, product.versions, product.alerts);
